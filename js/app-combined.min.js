...
/*! app.js */
$("#accounts ul").hide();$("#accounts p .current").on("focus",function(){$("#accounts ul").show()});$("#accounts p .current").on("keydown",function(a){if(a.keyCode==9){$("#accounts ul").addClass("tab")}});$("#accounts ul").on("mouseover",function(){$("#accounts ul").addClass("mouse")});$("#accounts ul").on("mouseout",function(){$("#accounts ul").removeClass("mouse").removeClass("tab")});$("#accounts ul li:last-child a").on("blur",function(){if($("#accounts ul").hasClass("tab")){$("#accounts ul").hide().removeClass("tab")}});$("#accounts p .current").on("blur",function(){if(!$("#accounts ul").is(".tab, .mouse")){$("#accounts ul").hide()}});function toggleButton(c){c.parent().children().removeClass("current");c.addClass("current");if(c.parent().attr("id")=="scale"){var e=c.text()=="Linear"?0:1;g.updateOptions({logscale:e})}else{if(c.parent().attr("id")=="period"){var b=new Date(),a=new Date(b.getFullYear(),b.getMonth(),1),d=new Date(b.getFullYear(),b.getMonth(),b.getDate());if(c.text()=="Current month"){zoomGraphX(a,d)}else{unzoomGraph()}}}}$("#options .button").on("click",function(){toggleButton($(this))});$("#options .button").on("keydown",function(a){if(a.keyCode==13||a.keyCode==32){toggleButton($(this))}});function toggleData(a){g.setVisibility(a.attr("value"),a.is(":checked")?1:0)}$("#options input").on("click",function(){toggleData($(this))});function setCurrentTab(){var a=window.location.href.split("#");if(a.length==1){$("#nav ul").children().removeClass("current");$("#nav a[href=#dashboard]").parent().addClass("current")}else{if(!a[1].length){}else{$("#nav ul").children().removeClass("current");$("#nav a[href="+window.location.hash+"]").parent().addClass("current")}}}function setCurrentOptions(){var a=window.location.hash;if(a=="#dashboard"){$("#options div").show().removeClass("hidden")}else{if(a=="#twitter"||a=="#identica"||a=="#facebook"){$("#options .network").hide().addClass("hidden");$("#options div").filter("."+a.substring(1)).show().removeClass("hidden")}}}function setCurrentGraph(){var a=window.location.hash.substring(1);if(a=="twitter"){g.updateOptions({visibility:[true,true,false,false,false,],series:{"Twitter followers":{axis:"y"},"Twitter tweets":{axis:"y2"},"Identica followers":{axis:"y"},"Identica notices":{axis:"y"},"Facebook likes":{axis:"y"},},})}else{if(a=="identica"){g.updateOptions({visibility:[false,false,true,true,false,],series:{"Twitter followers":{axis:"y"},"Twitter tweets":{axis:"y"},"Identica followers":{axis:"y"},"Identica notices":{axis:"y2"},"Facebook likes":{axis:"y"},},})}else{if(a=="facebook"){g.updateOptions({visibility:[false,false,false,false,true,],series:{"Twitter followers":{axis:"y"},"Twitter tweets":{axis:"y"},"Identica followers":{axis:"y"},"Identica notices":{axis:"y"},"Facebook likes":{axis:"y"},},})}else{if(a=="dashboard"){g.updateOptions({visibility:[true,true,true,true,true,],series:{"Twitter followers":{axis:"y"},"Twitter tweets":{axis:"y"},"Identica followers":{axis:"y"},"Identica notices":{axis:"y"},"Facebook likes":{axis:"y"},},})}}}}}g=new Dygraph(document.getElementById("graph"),localLog,{width:910,height:420,colors:["#13a7dd","#fdb524","#8dc500","#fb6104","#3b5998"],labelsSeparateLines:true,labelsDivStyles:{padding:"10px 15px",backgroundColor:"rgba(255, 255, 255, .85)",},visibility:graphVisibility,series:{"Twitter followers":{axis:"y"},"Twitter tweets":{axis:"y"},"Identica followers":{axis:"y"},"Identica notices":{axis:"y"},"Facebook likes":{axis:"y"},},axes:{y:{axisLabelFormatter:function(a){return Math.round(a)}},y2:{axisLabelFormatter:function(a){return Math.round(a)}}}});function zoomGraphX(a,b){g.updateOptions({dateWindow:[Date.parse(a),Date.parse(b)]})}function unzoomGraph(){g.updateOptions({dateWindow:null})}$(window).on("load",function(){setCurrentOptions();setCurrentTab();setCurrentGraph()});$(window).on("hashchange",function(){setCurrentOptions();setCurrentTab();setCurrentGraph()});